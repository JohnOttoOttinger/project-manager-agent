---
routine: datalabs-money-page-builder
trigger: trig_01A9etKicfxjFnHJNiRFWGCA
schedule: 0 19 * * * UTC  (5am AEST / 6am AEDT)
---

# The Datalabs Agency money-page builder - 5am
...
Read the Gmail thread whose subject starts `Datalabs money page candidates - ` (yesterday's date). Match **your own
...
- **no reply at all** - take the top `queued` item for this brand from
  `python3 skills/money-pages/scripts/suggest.py --list datalabs`, preferring origin `you`.
...
If Otto's reply was free text and no queue item matches, create one first:
    ...
    python3 skills/money-pages/scripts/suggest.py datalabs "<his words>"
...
Kit: `references/design-kit.html`. Model the script on the closest
`scripts/example-compose-datalabs-*.py`.

- Voice is "we/our". The brand is always **The Datalabs Agency**, with the name bolded.
- Prices are allowed ONLY where verified on the live workshop-pricing page or in brands.md.
...
## 5. Push as a draft
    ...
    ./skills/money-pages/scripts/wp-post.sh datalabs "<Page Title>" <composed.html>
...
cannot set (Custom background `#2f2e3a` + header style `2` (per-page tints exist - check the page record)); and the Yoast title and meta description to paste.
