...
stack and waits for healthy services. Backups made by the earlier Docker-based
release are still understood: their archive format and `env.backup` file are read
the same way, and because they contain no saved chats your current chats are left
unchanged.
